$(function(){
	...
	function slide(){
		...
		if(count%3==1){
			...
			$('#slide_1').css('background-color','#f70').siblings().css('background-color','#dedede');
		}else if(count%3==2){
			$('#pic_2').css('z-index',count);
			$('#slide_2').css('background-color','#f70').siblings().css('background-color','#dedede');
		}else{
			$('#pic_3').css('z-index',count);
			$('#slide_3').css('background-color','#f70').siblings().css('background-color','#dedede');
		}
	...
	//点击个人中心的时候如果没有登录则提示先登录再查看
	if($('#diff_module').length==0){
		...
		$('#personal_center').click(function(){
			$('#diff_module').fadeIn(100).fadeOut(100).fadeIn(100).fadeOut(100).fadeIn();
		});
	}
